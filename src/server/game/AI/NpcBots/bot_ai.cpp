...
bot_ai::bot_ai(Creature* creature) : CreatureAI(creature),
    //npcbot_plus
    _botData(const_cast<NpcBotData*>(BotDataMgr::SelectNpcBotData(0, creature->GetEntry()))),
    //end npcbot_plus
...
bool bot_ai::SetBotOwner(Player* newowner)
{
    ...
    //npcbot_plus
    _botData = const_cast<NpcBotData*>(BotDataMgr::SelectNpcBotData(newowner->GetGUID().GetCounter(), me->GetEntry()));
    LoadFromOwnerDB();
    //end npcbot_plus
...
void bot_ai::CheckOwnerExpiry()
{
    ...
    if (!_botData || _botData->owner == 0) //npcbot_plus
        return;
...
void bot_ai::ResetBotAI(uint8 resetType)
{
    ...
    {
        _checkOwershipTimer = (BotMgr::GetOwnershipExpireTime() && _botData && _botData->owner) ? (resetType == BOTAI_RESET_INIT) ? 1000 : CalculateOwnershipCheckTime() : 0; //npcbot_plus
            ...
            for (BotSpellMap::const_iterator itr = myspells.begin(); itr != myspells.end(); ++itr)
            {
                if (itr->first == basespell)
                {
                    ...
                    if (itr->second->enabled)
                        if (_botData) _botData->disabled_spells.erase(basespell); //npcbot_plus
                    else
                        if (_botData) _botData->disabled_spells.insert(basespell); //npcbot_plus
            ...
            switch (action - GOSSIP_ACTION_INFO_DEF)
            {
                case 1: //reset owner
                    ...
                    else
                    {
                        //npcbot_plus
                        uint32 newOwner = 0;
                        BotDataMgr::UpdateNpcBotData(0, me->GetEntry(), NPCBOT_UPDATE_OWNER, &newOwner);
                        //end npcbot_plus
...
bool bot_ai::GlobalUpdate(uint32 diff)
{
    ...
    if (_saveDisabledSpells && _saveDisabledSpellsTimer <= diff)
    {
        ...
        if (!IsTempBot())
        {
            //npcbot_plus
            if (_botData)
                BotDataMgr::UpdateNpcBotData(master->GetGUID().GetCounter(), me->GetEntry(), NPCBOT_UPDATE_DISABLED_SPELLS, &_botData->disabled_spells);
            //end npcbot_plus
        }
    }
    //  2) miscavalues
    if (_saveMiscValues && _saveMiscValuesTimer <= diff)
    {
        ...
        if (!IsTempBot())
            //npcbot_plus
            if (_botData)
                BotDataMgr::UpdateNpcBotData(master->GetGUID().GetCounter(), me->GetEntry(), NPCBOT_UPDATE_MISCVALUES, &_botData->miscvalues);
            //end npcbot_plus
    }
...
bool bot_ai::EquipOwnerItem(uint8 bagID, uint8 slotID, bool alt) {
    ...
    BotEquipResult equip = _equip(botSlot, item, master->GetGUID(), false);

    if (equip == BotEquipResult::BOT_EQUIP_RESULT_OK) {
        _unsavedChanges = true;
        UpdateVisuals();
    }

    return equip == BotEquipResult::BOT_EQUIP_RESULT_OK;
}
        ...
        if (Result) 
        {
            ...
            for (uint8 i = BOT_SLOT_MAINHAND; i != BOT_INVENTORY_SIZE; ++i)
            {
                ...
                if (ItemInstance) {
                    ...
                    _equip(i, item, ObjectGuid(), false);
                }
            }
...
void bot_ai::SaveToOwnerDB() {
    if (_unsavedChanges && master != NULL)
    {
        ...
        for (uint8 i = BOT_SLOT_MAINHAND; i != BOT_INVENTORY_SIZE; ++i)
        {
            ...
            if (Item)
            {
                if (Item->GetOwnerGUID() != master->GetGUID()) LOG_ERROR("server.loading", "Bot Item {} in slot {} does not belong to Bot Master.", Item->GetGUID().GetCounter(), i);
