...
public:
    ...
    static bool HandleNpcBotFixCommand(ChatHandler* handler, Optional<Variant<std::string_view, uint32>> bot_id_or_name)
    {
        ...
        NpcBotData const* bot_data = BotDataMgr::SelectNpcBotData(owner->GetGUID().GetCounter(), bot_id);
        //end npcbot_plus
    ...
    static bool HandleNpcBotFreeCommand(ChatHandler* handler)
    {
        ...
        BotDataMgr::UpdateNpcBotData(chr->GetGUID().GetCounter(), bot->GetEntry(), NPCBOT_UPDATE_SPEC, &spec); //npcbot_plus
        uint32 roleMask = bot_ai::DefaultRolesForClass(bot->GetBotClass(), spec);
        BotDataMgr::UpdateNpcBotData(chr->GetGUID().GetCounter(), bot->GetEntry(), NPCBOT_UPDATE_ROLES, &roleMask); //npcbot_plus

        if (!botowner)
        {
            uint32 newOwner = 0;
            BotDataMgr::UpdateNpcBotData(chr->GetGUID().GetCounter(), bot->GetEntry(), NPCBOT_UPDATE_OWNER, &newOwner); //npcbot_plus
