...
void BotDataMgr::Update(uint32 diff)
{
    ...
    {
        //npcbot_plus
        if (bot->IsFreeBot() && !bot->IsWandererBot() && !bot->IsInWorld() && bot->FindMap() && !!SelectNpcBotData(0, bot->GetEntry()))
        //end npcbot_plus
        {
...
void BotDataMgr::LoadNpcBots(bool spawn)
{
    ...
    if (!result)
    {
        ...
        BOT_LOG_INFO("server.loading", ">> Loaded {} bot data entries for deletion", datacounter);

        for (std::vector<uint32>::const_iterator itr = entryList.begin(); itr != entryList.end(); ++itr)
        {
            uint32 entry = *itr;
...
void BotDataMgr::UpdateNpcBotData(uint32 owner, uint32 entry, NpcBotDataUpdateType updateType, void* data)
{
    ...
    switch (updateType)
    {
        case NPCBOT_UPDATE_OWNER:
        {
            ...
            //"UPDATE characters_npcbot SET owner = ?, hire_time = FROM_UNIXTIME(?) WHERE entry = ? AND owner = ?", CONNECTION_ASYNC
            bstmt->SetData(0, itr->second->owner);
            bstmt->SetData(1, itr->second->hire_time);
            bstmt->SetData(2, entry);
            bstmt->SetData(3, owner);
            CharacterDatabase.Execute(bstmt);
        ...
        case NPCBOT_UPDATE_MISCVALUES:
        {
            ...
            //"UPDATE characters_npcbot SET miscvalues = ? WHERE entry = ? AND owner = ?", CONNECTION_ASYNCH
            bstmt->SetData(0, ss.str());
            bstmt->SetData(1, entry);
            bstmt->SetData(2, owner);
            CharacterDatabase.Execute(bstmt);
